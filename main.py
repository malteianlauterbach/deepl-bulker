    ...
    if request.method == 'POST':
        ...
        for file in files:
            ...
            if filename.endswith('.zip'):
                ...
                os.rename(os.path.join(UPLOAD_FOLDER, filename), os.path.join(QUEUE_FOLDER, filename))
                log_to_file(f'File Moved to Queue: {filename}')
            PROCESSED_COUNT += 1
        print(f"Total count of processed files: {PROCESSED_COUNT}")
        translated_files = translate_and_upload_documents()  # Call translation function after processing files

    ...
    with open(LOG_FILE, 'a') as log:
        for root, dirs, files in os.walk(QUEUE_FOLDER):
            for filename in files:
                ...
                try:
                    # Simulate uploading by waiting for 1 second
                    time.sleep(1)
                    # Save the file locally instead of uploading
                    shutil.copy(input_path, output_path)
                    timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                    log.write(
                        f"[{timestamp}] Simulated upload: '{filename}' saved as '{output_path}'\n"
                    )
                    ...
                    timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                    log.write(
                        f"[{timestamp}] Error processing document '{filename}': {error}\n"
                    )
